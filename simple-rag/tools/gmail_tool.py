...
(see that repo's tools/gmail_tool.py); this module's only job is:
  ...
  1. fetch this (user, bot) pair's current (auto-refreshed) Google
     access token from gmail_oauth.py, and
  2. call the matching MCP tool with that token attached as the
     X-Gmail-Access-Token header, for that one call only.

README.md Phase 4: build_gmail_tools now takes an optional `bot_id`,
threaded straight through to gmail_oauth.get_valid_access_token(user_id,
bot_id) -- bot_id=None reads the "no bot" connection (pre-Phase-4
behavior, unchanged), a real bot_id reads that bot's own independent
Gmail connection.
...
async def _call_gmail_mcp_tool(
    user_id: str, bot_id: str | None, tool_name: str, args: dict
) -> str:
    token = await asyncio.to_thread(gmail_oauth.get_valid_access_token, user_id, bot_id)
    if not token:
        return _wrap_untrusted("gmail", "Gmail is not connected for this bot.")
...
def _build_list_messages_tool(user_id: str, bot_id: str | None) -> BaseTool:
    ...
    async def list_gmail_messages(query: str = "", max_results: int = 10) -> str:
        ...
        log.info("list_gmail_messages user_id=%s bot_id=%s query=%r", user_id, bot_id, query)
        return await _call_gmail_mcp_tool(
            user_id, bot_id, "list_gmail_messages", {"query": query, "max_results": max_results}
        )

    return list_gmail_messages


def _build_get_message_tool(user_id: str, bot_id: str | None) -> BaseTool:
    ...
    async def get_gmail_message(message_id: str) -> str:
        ...
        log.info("get_gmail_message user_id=%s bot_id=%s message_id=%r", user_id, bot_id, message_id)
        return await _call_gmail_mcp_tool(
            user_id, bot_id, "get_gmail_message", {"message_id": message_id}
        )

    return get_gmail_message


def _build_send_message_tool(user_id: str, bot_id: str | None) -> BaseTool:
    ...
    async def send_gmail_message(to: str, subject: str, body: str) -> str:
        ...
        irreversible."""
        log.info(
            "send_gmail_message user_id=%s bot_id=%s to=%r subject=%r", user_id, bot_id, to, subject
        )
        return await _call_gmail_mcp_tool(
            user_id, bot_id, "send_gmail_message", {"to": to, "subject": subject, "body": body}
        )
...
async def build_gmail_tools(
    ...
    builds tools against the "no bot" Gmail connection, matching
